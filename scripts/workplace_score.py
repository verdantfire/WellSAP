...
def emp_satisfaction(dict):
    age=joblib.load('./scaler_and _encoder/emp_satisfaction_age_encoder.pkl')
    dict['Age_Group']=age.transform([dict['Age_Group']])

    gender=joblib.load('./scaler_and _encoder/emp_satisfaction_gender_encoder.pkl')
    dict['Gender']=gender.transform([dict['Gender']])

    overtime_encoder=joblib.load('./scaler_and _encoder/emp_satisfaction_overtime_encoder.pkl')
    dict['OverTime']=overtime_encoder.transform([dict['OverTime']])

    scaler=joblib.load('./scaler_and _encoder/scaler_emp_satisfaction.pkl')
    ...
    model=joblib.load('./models/workplace_wellbeing_emp_satisfaction.pkl')
    return int(model.predict(input_f)[0])

def env_satisfaction(dict):
    age=joblib.load('./scaler_and _encoder/env_age_encoder.pkl')
    dict['Age_Group']=age.transform([dict['Age_Group']])

    gender=joblib.load('./scaler_and _encoder/env_gender_encoder.pkl')
    dict['Gender']=gender.transform([dict['Gender']])

    job_diversity=joblib.load('./scaler_and _encoder/env_job_diversity_encoder.pkl')
    dict['FromDiversityJobFairID']=job_diversity.transform([dict['FromDiversityJobFairID']])

    citi_encoder=joblib.load('./scaler_and _encoder/env_citizendesc_encoder.pkl')
    dict['CitizenDesc']=citi_encoder.transform([dict['CitizenDesc']])

    his_encoder=joblib.load('./scaler_and _encoder/env_HispanicLatino_encoder.pkl')
    dict['HispanicLatino']=his_encoder.transform([dict['HispanicLatino']])

    race_encoder=joblib.load('./scaler_and _encoder/env_racedesc_encoder.pkl')
    dict['RaceDesc']=race_encoder.transform([dict['RaceDesc']])

    attri_encoder=joblib.load('./scaler_and _encoder/env_attri_encoder.pkl')
    dict['Attrition']=attri_encoder.transform([dict['Attrition']])

    busi_encoder=joblib.load('./scaler_and _encoder/env_busi_encoder.pkl')
    dict['BusinessTravel']=busi_encoder.transform([dict['BusinessTravel']])

    scaler=joblib.load('./scaler_and _encoder/env_minmax_scaler.pkl')
    ...
    model=joblib.load('./models/workplace_wellbeing_env_satisfaction.pkl')
    input_f = [[dict[key] for key in dict.keys()]]
    return int(model.predict(input_f)[0])
